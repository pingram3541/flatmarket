...
<?php if(!$this->repository_has_subscription($repository_id) || !empty($subscription_no_match)): ?>            
            ...
            <a class="button-primary" style="margin-bottom:10px;" href="<?php echo $this->append_parameters_to_buy_url($product['url'], $args) ?>"><?php printf(__('Buy %s', 'installer'), $args['product_name']) ?></a> 
            
            <div>
                <br />
                <?php printf(__('Already bought %s?', 'installer'), $args['product_name']) ?>
                <a class="enter_site_key_js button-secondary" style="margin-top:-5px;" href="#"><?php _e('Enter site key', 'installer') ?></a> (You can get your site key in <a href="https://wpml.org/account/sites/" target="_blank">WPML Account > Sites</a>).
...
<?php else: ?>
    ...
    <center>
    <a class="remove_site_key_js button-secondary" href="#" data-repository=<?php echo $repository_id ?> data-confirmation="<?php esc_attr_e('Are you sure you want to remove this site key?', 'installer') ?>" data-nonce="<?php echo wp_create_nonce('remove_site_key_' . $repository_id) ?>"><?php printf(__("Remove current site key (%s)", 'installer'), $site_key) ?></a>
    </center>
    <br />
