...
if (!class_exists('IPANEL')) {
    ...
    class IPANEL
    {
        ...
        {
            if ($var['type'] == 'EndTab' || $var['type'] == 'export' || $var['type'] == 'import' || $var['type'] == 'wpmlinstaller' || $var['type'] == 'StartSection' || $var['type'] == 'EndSection' || $var['type'] == 'info') {
                return true;
            }
        ...
        public function generateFields($fields)
        {
            ...
            foreach ($f as $item) {
                ...
                case ('wpmlinstaller'):
                    $output .= $this->wpmlinstaller($item);
                    break;
        ...
        { 
            ...
            if ( false === ( get_transient( 'theme_googleFonts'  ) ) ) {
                ...
                $settings_array = @unserialize(get_option('FLATMARKET_PANEL'));
                  
                $fonts_api_key = $settings_array['font_api_key'];

                if(trim($fonts_api_key == '')) {
                  $fonts_api_key = 'AIzaSyBFSLcqgaFj_DgnHM-_Xfh3sOlxfxl7SOI'; // Default key
                }
                ...
                if (!is_wp_error($response)) {
                    ...
                    set_transient('theme_googleFonts', $fonts_list, 60 * 60 * 24 * 2); // Cache 2 days
                            ...
                            <!-- Left Side-->
                                <div id="ipanel-tabs">
                                    <ul>
                                        ...
                                        <!-- WPML Installer -->
                                        <li id="tab-wpml_installer"><a data-target="wpml_installer" href="#"><?php _e('Multilingual (WPML)', $this->text_domain); ?></a></li>
                                        <!-- // WPML Installer -->
                                    </ul>
                                </div>
                                <!-- Left Side-->
                        ...
                        <td id="ipanel-fields-container">
                            <div>
                                <!-- Fields Container -->
                                    ...
                                    <!-- WPML Installer -->
                                    <div id="ipanel-wpml_installer" class="ipanel-tabs">
                                      <div class="ipanel-wpmlinstaller-field ipanel-field-section">
                                        <div class="ipanel-label">
                                          <label><?php _e('WPML Plugin Installer', $this->text_domain); ?></label>
                                        </div>
                                        <div class="ipanel-input">
                                      <div class="ipanel-wpml-installer">
                                      <?php 
                                        $config['template'] = 'compact'; //required
                                        $config['product_name'] = 'WPML';
                                        $config['box_title'] = 'Enable Multilingual and Multicurrency support for FlatMarket theme:';
                                        $config['name'] = 'FlatMarket'; //name of theme/plugin
                                        $config['box_description'] = '<div class="ipanel-info alert">FlatMarket theme is fully compatible with WPML ­the WordPress Multilingual plugin and even <a href="http://marketblog.envato.com/envatos-most-wanted/winners-wpml-woocommerce-wordpress-themes/" target="_blank"><b>won first place</b> in WPML Best WordPress themes contest in 2014</a>. <br>WPML plugin allow you to completely translate your website, make it multilanguage / multicurrency and includes advanced translation management in few clicks. <br>WPML plugin should be purchased separately from theme.</div>';
                                        $config['repository'] = 'wpml'; // required 
                                        $config['package'] = 'multilingual-cms'; // required 
                                        $config['product'] = 'multilingual-cms'; // required
                                        
                                        $op = '';
                                        $op .= '<div class="ipanel-wpml-installer">';
         ...
         * @since 1.0
         */
        public function wpmlinstaller($options = null)
        { 
